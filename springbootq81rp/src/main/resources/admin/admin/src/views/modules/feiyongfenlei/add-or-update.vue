<template>
	<div class="addEdit-block" style="width: 100%;">
		...
		>
			<template >
				<el-form-item :style='{"width":"45%","margin":"0 2% 20px 0","display":"inline-block"}' class="input" v-if="type!='info'"  label="费用分类" prop="feiyongfenlei">
					<el-input v-model="ruleForm.feiyongfenlei" placeholder="费用分类" clearable  :readonly="ro.feiyongfenlei"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"45%","margin":"0 2% 20px 0","display":"inline-block"}' v-else class="input" label="费用分类" prop="feiyongfenlei">
					<el-input v-model="ruleForm.feiyongfenlei" placeholder="费用分类" readonly></el-input>
				</el-form-item>
			</template>
...
export default {
	data() {
		...
		return {
			...
			rules: {
				feiyongfenlei: [
					{ required: true, message: '费用分类不能为空', trigger: 'blur' },
				],
			}
		};
	},
	...
	methods: {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						if(o=='feiyongfenlei'){
							this.ruleForm.feiyongfenlei = obj[o];
							this.ro.feiyongfenlei = true;
							continue;
						}
				}
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `feiyongfenlei/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.feiyongfenleiCrossAddOrUpdateFlag = false;
						 this.parent.search();
